...
public static class ModUpdater
{
    ...
    public async static void CheckForUpdates(string path, MainWindow main)
    {
        ...
        foreach (var mod in mods)
        {
            ...
            try
            {
                ...
                metadata = JsonSerializer.Deserialize<Metadata>(metadataString)
                    ?? throw new InvalidDataException("Mod metadata is empty.");
            }
            catch (Exception e)
            {
            ...
            if (url != null)
            {
                ...
                if (!requestUrls.ContainsKey(MOD_TYPE))
                    requestUrls.Add(MOD_TYPE, new List<string> { $"https://gamebanana.com/apiv6/{MOD_TYPE}/Multi?_csvProperties=_sName,_aSubmitter,_aCategory,_aSuperCategory,_sProfileUrl,_sDescription,_bHasUpdates,_aLatestUpdates,_aFiles,_aPreviewMedia,_aAlternateFileSources,_tsDateUpdated&_csvRowIds=" });
        ...
        if (requestUrls.Count == 0)
        {
            ...
            main.LoadoutsBox.IsEnabled = true;
            main.ModGridSearchButton.IsEnabled = true;
            if (!Global.CurrentGame.Equals("Dragon Ball FighterZ", StringComparison.InvariantCultureIgnoreCase))
                main.LauncherOptionsBox.IsEnabled = true;
            return;
        }
        List<GameBananaAPIV4> response = [];
    ...
    private static async Task ModUpdate(GameBananaAPIV4 item, string mod, Metadata metadata, Progress<DownloadProgress> progress, CancellationTokenSource cancellationToken)
    {
        ...
        {
            if (item.HasUpdates == true && item.Updates is { Length: > 0 })
        ...
        if (item.HasUpdates == true && item.Updates is { Length: > 0 })
        {
            ...
            if (DateTime.Compare(metadata.lastupdate.Value, update.DateAdded) < 0)
            {
                ++updateCounter;
                ...
                ChangelogBox changelogBox = new(update, Path.GetFileName(mod), $"A new update is available for {Path.GetFileName(mod)}", item.Image, true);
                ...
                if (files.Count > 1)
                {
                    UpdateFileBox fileBox = new(files.ToList(), Path.GetFileName(mod));
                    fileBox.Activate();
                    fileBox.ShowDialog();
                ...
                if (item.AlternateFileSources != null)
                {
                    ...
                    {
                        if (Global.CurrentGame is { } game && metadata.homepage is { } homepage)
                            new AltLinkWindow(item.AlternateFileSources, Path.GetFileName(mod), game, homepage.AbsoluteUri, true).ShowDialog();
                        return;
                    }
                }
        ...
        await Task.Run(() =>
        {
            ...
            if (File.Exists(_ArchiveSource))
            {
                try
                {
                    ...
                    {
                        using var archive = SevenZipArchive.OpenArchive(_ArchiveSource);
