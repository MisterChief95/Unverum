...
public class ModDownloader
{
    ...
    public async void BrowserDownload(string game, GameBananaRecord record)
    {
        ...
        if (downloadWindow.YesNo)
        {
            ...
            {
                UpdateFileBox fileBox = new(record.AllFiles, record.Title);
                fileBox.Activate();
                fileBox.ShowDialog();
    ...
    private async Task ExtractFile(string fileName, string game, GameBananaRecord record)
    {
        await Task.Run(() =>
        {
            game = game switch
            {
                "Demon Slayer The Hinokami Chronicles" => "Demon Slayer",
                "THE IDOLM@STER STARLIT SEASON" => "IDOLM@STER",
