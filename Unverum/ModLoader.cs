...
public static class ModLoader
{
    ...
    public static bool Restart(string path, string movies, string splash, string sound)
    {
        try
        {
            ...
            if (Global.CurrentGame.Equals("Dragon Ball Sparking! ZERO", StringComparison.InvariantCultureIgnoreCase))
            {
            ...
            if (!string.IsNullOrEmpty(Global.CurrentGameConfig.PatchesFolder))
            {
                if (Directory.Exists(Global.CurrentGameConfig.PatchesFolder))
                    Directory.Delete(Global.CurrentGameConfig.PatchesFolder, true);
                Directory.CreateDirectory(Global.CurrentGameConfig.PatchesFolder);
                // Delete sound folder in romfs if it exists
                var SwitchSound = Global.CurrentGameConfig.PatchesFolder.Replace("exefs", $"romfs{Global.s}Project{Global.s}Content{Global.s}Sound");
            ...
            List<string> ue4ssFiles = ["opengl32.dll", "patternsleuth_bind.dll", "ue4ss.dll", "UE4SS-settings.ini", "dwmapi.dll"];
    ...
    private static int CopyFolder(Dictionary<string, bool> paks, string sourcePath, string targetPath, string? defaultSig)
    {
        ...
        foreach (var path in Directory.GetFiles(sourcePath, "*.*", SearchOption.AllDirectories))
        {
                ...
                && paks.ContainsKey(path) && paks[path])
            {
                ...
                Directory.CreateDirectory(Path.GetDirectoryName(newPath) ?? targetPath);
        ...
        if (Global.CurrentGame.Equals("Shin Megami Tensei V", StringComparison.InvariantCultureIgnoreCase))
            foreach (var path in Directory.GetDirectories(sourcePath, "*.*", SearchOption.AllDirectories))
            {
                var SoundFolder = Global.CurrentGameConfig.PatchesFolder?.Replace("exefs", $"romfs{Global.s}Project{Global.s}Content{Global.s}Sound");
                if (SoundFolder == null)
                    return counter;
                if (Path.GetFileName(path).Equals("Sound", StringComparison.InvariantCultureIgnoreCase))
                {
                    foreach (var file in Directory.GetFiles(path, "*", SearchOption.AllDirectories))
                    {
                        ...
                        Directory.CreateDirectory(Path.GetDirectoryName(newPath) ?? SoundFolder);
    ...
    private static void PakFiles(string path, string output, string? sig)
    {
        ProcessStartInfo startInfo = new()
        {
            ...
            FileName = $"{Global.assemblyLocation}{Global.s}Dependencies{Global.s}u4pak{Global.s}u4pak.exe",
            WindowStyle = ProcessWindowStyle.Hidden,
            WorkingDirectory = $"{Global.assemblyLocation}{Global.s}Dependencies{Global.s}u4pak",
            Arguments = $"pack \"{output}{Global.s}Unverum_9_P.pak\" {path}"
        };
    ...
    private static void CopyDirectoryWithRename(string sourceDir, string destDir)
    {
        foreach (string sourceFilePath in Directory.GetFiles(sourceDir))
        {
            ...
            Directory.CreateDirectory(Path.GetDirectoryName(destFilePath) ?? destDir);
    ...
    public static void Build(string path, List<Mod> mods, bool? patched, string movies, string splash, string sound)
    {
        ...
        string? sig = null;
        var modsPath = Path.GetDirectoryName(path) ?? throw new ArgumentException("The output path must have a parent directory.", nameof(path));
        var sigs = Directory.GetFiles(modsPath, "*.sig", SearchOption.TopDirectoryOnly);
        if (sigs.Length > 0)
            sig = sigs[0];
        var folderLetter = 'a';
        var tildes = 0;
        ...
        foreach (var mod in mods)
        {
            ...
            var modPath = $@"{Global.assemblyLocation}{Global.s}Mods{Global.s}{Global.CurrentGame}{Global.s}{mod.name}";
            ...
            if (uplugins.Any())
            {
                SZMods = uplugins.Select(Path.GetDirectoryName).OfType<string>().ToList();
            ...
            foreach (var ue4ssModDirectory in Directory.GetDirectories(modPath, "*ue4ss", SearchOption.AllDirectories))
                foreach (var ue4ssMod in Directory.GetFiles(ue4ssModDirectory, "*", SearchOption.AllDirectories))
                {
                    ...
                    Directory.CreateDirectory(Path.GetDirectoryName(newPath) ?? ue4ssModsFolder);
            ...
            foreach (var logicModDirectory in Directory.GetDirectories(modPath, "*LogicMods", SearchOption.AllDirectories))
                foreach (var logicMod in Directory.GetFiles(logicModDirectory, "*", SearchOption.AllDirectories))
                {
                    ...
                    Directory.CreateDirectory(Path.GetDirectoryName(newPath) ?? LogicModsFolder);
                ...
                switch (ext)
                {
                    case ".txt":
                        if (Path.GetFileName(file).Equals("dblist.txt", StringComparison.InvariantCultureIgnoreCase) &&
                            Global.CurrentGame.Equals("My Hero One's Justice 2", StringComparison.InvariantCultureIgnoreCase))
                        {
                    ...
                    case ".pchtxt":
                    case ".ips":
                        if (!string.IsNullOrEmpty(Global.CurrentGameConfig.PatchesFolder) && Directory.Exists(Global.CurrentGameConfig.PatchesFolder))
                            File.Copy(file, $"{Global.CurrentGameConfig.PatchesFolder}{Global.s}{Path.GetFileName(file)}", true);
                        break;
                    case ".json":
                        ...
                        if (Path.GetFileName(file).Equals("text.json", StringComparison.InvariantCultureIgnoreCase) &&
                                (Global.CurrentGame.Equals("Dragon Ball FighterZ", StringComparison.InvariantCultureIgnoreCase)
                                || Global.CurrentGame.Equals("Guilty Gear -Strive-", StringComparison.InvariantCultureIgnoreCase)
                                || Global.CurrentGame.Equals("Granblue Fantasy Versus", StringComparison.InvariantCultureIgnoreCase)
                                || Global.CurrentGame.Equals("Granblue Fantasy Versus Rising", StringComparison.InvariantCultureIgnoreCase)
                                || Global.CurrentGame.Equals("DNF Duel", StringComparison.InvariantCultureIgnoreCase)))
                        {
                            if (missing)
                                continue;
                            var pakName = Global.CurrentGame.Equals("DNF Duel", StringComparison.InvariantCultureIgnoreCase) ? "RED-WindowsNoEditor.pak" : "pakchunk0-WindowsNoEditor.pak";
            ...
            if (Global.CurrentGame.Equals("Jump Force", StringComparison.InvariantCultureIgnoreCase))
        ...
        if (db != null)
        {
            ...
            Directory.CreateDirectory(Path.GetDirectoryName(dbOutput) ?? Global.assemblyLocation);
        ...
        if (patched != null && (bool)patched && Global.CurrentGame != "Scarlet Nexus" && Global.CurrentGame != "Dragon Ball Sparking! ZERO")
        {
            var baseFolder = $"{path}{Global.s}--Base--";
            using (var resource = Assembly.GetExecutingAssembly().GetManifestResourceStream($"Unverum.Resources.Patches.{Global.CurrentGame.Replace(" ", "_").Replace("-", "_")}.Placeholder.--PlaceholderCostumes.pak"))
            {
                if (resource == null)
                    throw new InvalidOperationException("Costume placeholder resource is missing.");
        ...
        if (!string.IsNullOrEmpty(prmFilePaths))
        {
            ...
            ProcessStartInfo startInfo = new()
            {
                CreateNoWindow = true,
                UseShellExecute = false,
                FileName = $"{Global.assemblyLocation}{Global.s}Dependencies{Global.s}jfaddslots{Global.s}JFAddSlots.exe",
                WindowStyle = ProcessWindowStyle.Hidden,
                WorkingDirectory = $"{Global.assemblyLocation}{Global.s}Dependencies{Global.s}jfaddslots",
                Arguments = $@"""{Global.assemblyLocation}{Global.s}Dependencies{Global.s}u4pak"" {prmFilePaths}"
            };
            using (Process process = new())
            {
                process.StartInfo = startInfo;
