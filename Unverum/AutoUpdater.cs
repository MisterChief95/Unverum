...
public class AutoUpdater
{
    ...
    public static async Task<bool> CheckForUnverumUpdate(CancellationTokenSource cancellationToken)
    {
        ...
        try
        {
            ...
            if (response.HasUpdates != null && (bool)response.HasUpdates)
            {
                ...
                string updateTitle = updates[0].Title ?? string.Empty;
                ...
                {
                    ChangelogBox notification = new(updates[0], "Unverum", $"A new version of Unverum is available (v{onlineVersion})!", null!);
                    notification.ShowDialog();
                    notification.Activate();
                    if (notification.YesNo)
                    {
                        Dictionary<string, GameBananaItemFile> files = response.Files!;
                        string downloadUrl = files.ElementAt(0).Value.DownloadUrl ?? string.Empty;
                        string fileName = files.ElementAt(0).Value.FileName ?? string.Empty;
                        // Download the update
                        ...
                        // Update Unverum
                        UpdateManager updateManager = new(new LocalPackageResolver($"{Global.assemblyLocation}{Global.s}Downloads{Global.s}UnverumUpdate"), new ZipExtractor());
    ...
    private static bool UpdateAvailable(string? onlineVersion, string? localVersion)
    {
        ...
        if (onlineVersionParts.Length > localVersionParts.Length)
        {
            ...
            {
                localVersionParts = [.. localVersionParts, "0"];
            }
        }
        else if (localVersionParts.Length > onlineVersionParts.Length)
        {
            ...
            {
                onlineVersionParts = [.. onlineVersionParts, "0"];
            }
        }
